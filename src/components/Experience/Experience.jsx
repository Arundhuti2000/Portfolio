...
const getLogoPath = (company) => {
  ...
  return (
    <section id="experience" className="py-20 md:py-28 lg:py-36 bg-gradient-to-b from-gray-950 to-black">
      <div className="max-w-7xl mx-auto px-4">
        <FadeInSection>
          <h2 className="text-5xl md:text-6xl font-bold text-center mb-24 text-white">Professional Journey</h2>
        </FadeInSection>
        ...
        <div className="hidden md:block relative">
          ...
          <div className="relative h-[800px]">
            ...
            <div className="absolute top-1/2 left-0 transform -translate-y-1/2 flex items-center z-20">
              <span className="text-white font-semibold text-lg mr-3">2020</span>
            ...
            <div className="absolute top-1/2 h-0.5 bg-gradient-to-r from-purple-500 via-blue-500 via-indigo-500 to-cyan-500 transform -translate-y-1/2 z-0" 
                 style={{ left: '54px', right: '54px' }}></div>
                ...
                return (
                  <FadeInSection key={exp.id} delay={index * 100}>
                    <div className="relative h-full flex flex-col items-center justify-center">
                      ...
                      <div
                        className={`absolute w-full max-w-[350px] bg-gray-900/90 rounded-lg border ${colors.border}/30 p-6 ${colors.hover} hover:shadow-lg transition-all duration-300 ${
                          isEven ? "bottom-1/2 mb-16" : "top-1/2 mt-16"
                        }`}
                      >
                        <div className="text-center mb-3">
                          <h3 className="text-lg font-bold text-white mb-1">{exp.title}</h3>
                          <p className={`${colors.text} font-semibold text-sm`}>{exp.company}</p>
                        ...
                        <ul className="space-y-1 text-xs">
                          {exp.points.map((point, pointIndex) => (
                            <li key={pointIndex} className="flex items-start text-gray-300">
                              <span className={`${colors.text} mr-1 mt-1 text-xs`}>•</span>
                              <span>{point}</span>
                            </li>
                          ))}
                        </ul>
            ...
            return (
              <FadeInSection key={exp.id} delay={index * 100}>
                <div className="flex items-start space-x-4">
                  ...
                  {/* Content */}
                  <div className={`bg-gray-900/90 rounded-lg border ${colors.border}/30 p-4 flex-1`}>
                    <div className="mb-3">
                      <h3 className="text-lg font-bold text-white mb-1">{exp.title}</h3>
                      <p className={`${colors.text} font-semibold text-sm`}>{exp.company}</p>
                      <p className="text-gray-400 text-xs">{exp.duration}</p>
